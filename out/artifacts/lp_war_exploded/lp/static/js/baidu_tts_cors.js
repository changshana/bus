...
function btts(param, options) {
    var url = window.location.protocol+'//tsn.baidu.com/text2audio';
    var opt = options || {};
    var p = param || {};
    // 默认超时时间10秒
        ...
        if (xhr.readyState == 4) {
            ...
            if (xhr.status == 200 ) {
                if (xhr.response.type === 'audio/mp3') {
                    ...
                    if (opt.autoDestory) {
                        audio.onended = function() {//监听音频播放结束触发事件   提示语音频播放那个结束后，播放正文音频
                            document.body.removeChild(audio);
                            if (!(opt.l.ie && opt.l.ie < 9)) {
                                if (opt.i.isPlay) {
                                    var a = document.getElementById("mainAudio");
                                    if(a!=null){
                                        a.src = opt.i.audioSrc, a.play();
                                    }else{
                                        var i = document.createElement("audio");
                                        i.setAttribute('id', 'mainAudio');
                                        document.body.appendChild(i), i.src = opt.i.audioSrc, i.play();
                                    }
                                }
                            }
                        }
