...
layui.define(['jquery','layer'],function(exports){
    ...
    Class.prototype.init = function(){
        ...
        if(options.aldaBtn){
            $(options.elem).parent().after(spantext).after('<input type="hidden" id="inputTagsData"/>')
        }
    ...
    Class.prototype.events = function(){
        ...
        $('.albtn').on('click',function(){
            alert(options.content+"(即id为inputTagsData的input组件的值)")
            $('#inputTagsData').val(options.content);
        })
...
}).link('lib/layui2.4.1/extend/inputTags/inputTags.css')