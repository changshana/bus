...
;layui.define(["layer", "laytpl", "upload"], function (i) {
    ...
    var u = function (i) {
        ...
        f = ['<div class="chatbox-top"><div class="chatbox-top-left"><i class="i"></i><span class="username">{{ d.data.name||"佚名" }}</span><span class="sign">{{ d.data.sign||"佚名" }}</span></div>'
            ,'<div class="chatbox-top-right"><div class="navi"><i></i><span>快速导航</span></div><div class="setting"><i></i><span>设置</span></div></div> </div>'
            ,'<div class="guide" ></div>'
            ,'<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">', '<div class="layui-unselect layim-chat-title">', '<div class="layim-chat-other">', '<!--<img class="layim-{{ d.data.type }}{{ d.data.id }}" src="{{ d.data.avatar }}">--><span class="layim-chat-username" layim-event="{{ d.data.type==="group" ? "groupMembers" : "" }}">{{ d.data.name||"佚名" }}{{ d.data.version||"佚名" }}版 {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>', '<p class="layim-chat-status"></p>', "</div>",'<div class="chat-other-right"><div class="new-func"><i></i><span>新增功能</span></div><div class="help"><i></i><span>帮助中心</span></div></div>','<span class="layui-fullSrceen" onclick="launchFullscreen(this,document.documentElement);"><a class="fullSrceen-icon" href="javascript:;"></a></span>', "</div>", '<div class="layim-chat-main">', "<ul></ul>", "</div>",'<div class="shortcuts"></div>','<div class="recorder"></div>','<div class="layim-chat-footer">', '<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">', '<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face">&#xe60c;</span>', "{{# if(d.base && d.base.uploadImage){ }}", '<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file"></span>', "{{# }; }}", "{{# if(d.base && d.base.uploadFile){ }}", /*'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>', */"{{# }; }}", "{{# if(d.base && d.base.isAudio){ }}", '<span id="layim-tool-audio" class="layui-icon layim-tool-audio" title="发送语音消息" layim-event="media" data-type="audio">&#xe6fc;</span>', "{{# }; }}", "{{# if(d.base && d.base.isVideo){ }}", /*'<span class="layui-icon layim-tool-video" title="发送网络视频" layim-event="media" data-type="video">&#xe6ed;</span>',*/ "{{# }; }}", "{{# layui.each(d.base.tool, function(index, item){ }}", '<span class="layui-icon layim-tool-{{item.alias}}" title="{{item.title}}" layim-event="extend" lay-filter="{{ item.alias }}">{{item.icon}}</span>', "{{# }); }}", "{{# if(d.base && d.base.chatLog){ }}", /*'<span class="layim-tool-log" layim-event="chatLog"><i class="layui-icon">&#xe60e;</i>聊天记录</span>',*/ "{{# }; }}", "</div>", '<div class="layim-chat-textarea"><textarea></textarea></div>', '<div class="layim-chat-bottom">', '<div class="layim-chat-send">', "{{# if(!d.base.brief){ }}", /*'<span class="layim-send-close" layim-event="closeThisChat">关闭</span>', */"{{# } }}", '<span class="layim-send-btn" layim-event="send">发送</span>', '<span class="layim-send-set" layim-event="setSend" lay-type="show"><em class="layui-edge"></em></span>', '<ul class="layui-anim layim-menu-box">', '<li {{d.local.sendHotKey !== "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend" lay-type="Enter"><i class="layui-icon layui-icon-ok"></i>按Enter键发送消息</li>', '<li {{d.local.sendHotKey === "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend"  lay-type="Ctrl+Enter"><i class="layui-icon layui-icon-ok"></i>按Ctrl+Enter键发送消息</li>', "</ul>", "</div>", "</div>", "</div>", "</div>"].join(""),
        ri = ['<div class="layim-chat-right">',
            '<div class="sidebar-tool sidebar-tool-draw tool-wireless" data-toolname="wireless">','<div class="sidebar-tool-header">','<div class="icon wireless-icon1"></div><span class="header">无线版</span><span class="badge">推荐</span><i class="icon layui-icon">&#xe61a;</i>','</div>','<div class="sidebar-tool-content-wrapper">','<div class="sidebar-tool-content"><div class="qr-code-wrapper"></div><ul class="feature"><li>极速响应</li><li>24小时在线</li><li>随时随地,一键触达</li></ul></div>','</div>', '</div>',
            '<div class="sidebar-tool sidebar-tool-draw tool-hotline" data-toolname="wireless">','<div class="sidebar-tool-header"><div class="icon wireless-icon2"></div><span class="header">服务热线</span><span class="badge">忙碌</span><i class="icon layui-icon">&#xe61a;</i></div>','<div class="sidebar-tool-content-wrapper" ><div class="sidebar-tool-content"><ul><li><span class="label">客服1：</span><span class="phone">123456</span></li><li><span class="label">客服2：</span><span class="phone">45434</span></li></ul><div class="contact-link-wrapper">  <a href="#" target="_blank">更多服务热线</a></div></div> </div>','</div>','</div>'].join(""),
    ...
    layui.data.date = function (i) {
        ...
        var a = function (i) {
            return new RegExp("\\n*\\[" + (i || "") + "(code|pre|div|span|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5|a|img|i|button|br|strong)([\\s\\S]*?)\\]\\n*", "g")
        };
        return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/face\[([^\s\[\]]+?)\]/g, function (i) {
    ...
    var x, b, w, k, C, S = function (i, a, n) {
        ...
        var u = C = t.open({
            ...
            content: n('<ul class="layui-unselect layim-chat-list">' + v + '</ul><div class="layim-chat-left"><div class="logo-tips"><span class="tips-title">我是智能秘书，我是棉棉</span></div><div class="mht-logo-side"><img class="mht-logo" src="{{ d.data.logo}}"></div><div class="weather"></div></div><div class="layim-chat-box">' + f + '</div>'+ri).render(l),
            success: function (a) {
                w = a, a.css({
                    "min-width": "500px",
                    "min-height": i.height,
        ...
        if (!(l.ie && l.ie < 9)) {
            if(i.isPlay){
                var a = document.getElementById("mainAudio");
                if(a!=null){
                    a.src = i.audioSrc, a.play();
                }else{
                    a= document.createElement("audio");
                    a.setAttribute('id', 'mainAudio');
                    document.body.appendChild(a), a.src =i.audioSrc, a.play();
                }
            }
        ...
        if(i.cluewords==undefined || i.cluewords==null || i.cluewords==""){
            Aue(i);//语音播报正文
            return ;
        }else{//语音播报提示语和正文
            audio = btts({
        ...
        if (w) {
            ...
            i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && c.eq(0).append(n(h).render(i)), Z()
            ...
            // i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && c.append(li), Z()
        }
        ...
        if (t >= d) {
            ...
            e.prev().hasClass("layim-chat-system") || e.eq(0).before('<div class="layim-chat-system"><span layim-event="chatLog">查看更多记录</span></div>'), t > d && n.remove()
        }
        ...
        status: function (i, a) {
            ...
            n = '<ul class="layui-clear layim-face-list">' + n + "</ul>", ti.face.index = t.tips(n, i, {
                ...
                success: function (i) {
                    i.find(".layim-face-list>li").on("mousedown", function (i) {
                        ...
                        pattern=false;
                        ai(l.textarea[0], "face" + this.title + " "), t.close(ti.face.index),D()
                    })
                }
            ...
            if("音频"==n[a]){
                layui.layim.setChatStatus('<span style="color:#FF5722;">正在录音...</span>');
                initRecorder();
                var $=layui.$;
                $(".shortcuts").hide();
                var ter;
                var sec=0;//录音定时,最长时间60s
                //var element = layui.element;
                HZRecorder.get(function (rec) {
                    recorder = rec;
                    recorder.start();
                });
                //document.getElementsByClassName("layui-layer-btn0")[0].addClass("layui-btn-disabled");
                ter=setInterval(function(){
                    if(sec>=60){
                        layer.tips('录音时长最大60s', '#tipMsg',{
                            tips:1
                        });
                        clearInterval(ter);//进度条停止变动
                        recorder.stop();//停止录音
                        return false;
                    }
                    sec+=1;
                    element.progress('recordAudio',Math.ceil(sec*100/60)+'%');
                    document.getElementById("recordTime").innerHTML=sec+'s';
                },1000);
                ...
                $("#sendAudio").click(function(){
                    ...
                    recorder.stop();
                    clearInterval(ter);
                    recorder.upload("xim/uploadAudio", function (state, e) {
                        switch (state) {
                            case 'uploading':
                            ...
                            case 'ok':
                                ...
                                if(isNotEmpty(res)){
                                    res=eval("("+res+")");
                                    //ai(l.textarea[0], a + "[" + res.result.path + "]"), D();
                                    if(res.result.txt!="error"){
                                        ai(l.textarea[0], res.result.txt), D();
                                    }else{
                                        var i={cluewords:'小棉没有挺清楚您讲的什么，请大声一点！谢谢'};
                                        Au(i);
                                    }
                                    //t.close(e);
                                }
                ...
                $("#cancelAudio").click(function(){
                    ...
                    recorder.stop();
                    clearInterval(ter);
                    element.progress('recordAudio','0%');
