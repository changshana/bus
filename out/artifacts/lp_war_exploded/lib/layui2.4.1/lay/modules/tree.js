...
;layui.define("jquery", function (e) {
    ...
    r.prototype.init = function (e) {
        ...
        layui.each(n, function (a, n) {
            var l = n.children && n.children.length > 0,
                ...
                s = o(["<li " + (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">", function () {
                    ...
                    return (l ? ('<a href="' + (n.href || "javascript:;") + '" ' + (r.target && n.href ? 'target="' + r.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite>")) :
                        ('<a href="' + (n.href || "javascript:;") + '" ' + (r.target && n.href ? 'target="' + r.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite>")
                        ))
                }(), (r.add==true?'<span class="tree_btn tree_add" title="添加节点下级"><i class="layui-icon btn_icon layui-icon-add-circle"></i></span>':'') +
                (r.edit==true?'<span class="tree_btn tree_edit" title="编辑节点信息"><i class="layui-icon btn_icon layui-icon-edit"></i></span>':'') +
                (r.delete==true?'<span class="tree_btn tree_delete" title="删除该节点"><i class="layui-icon btn_icon layui-icon-delete"></i></span>':'')+"</a></li>"].join(""));
            l && (s.append(c), i.tree(c, n.children)), e.append(s), "function" == typeof r.click && i.click(s, n), i.spread(s, n), r.drag && i.drag(s, n),i.addClick(s,n),i.editClick(s,n),i.deleteClick(s,n)
        })
    }, r.prototype.addClick = function (e, o) {
        var a = this, i = a.options,obj=e;
        e.children("a").find("span.tree_add").on("click", function (e) {
            layui.stope(e), i.addClick(o,obj)
        })
    }, r.prototype.editClick = function (e, o) {
        var a = this, i = a.options,obj=e;
        e.children("a").find("span.tree_edit").on("click", function (e) {
            layui.stope(e), i.editClick(o,obj)
        })
    }, r.prototype.deleteClick = function (e, o) {
        var a = this, i = a.options,obj=e;
        e.children("a").find("span.tree_delete").on("click", function (e) {
            layui.stope(e), i.deleteClick(o,obj)
        })
        ...
        var a = this, i = (a.options, e.children(".layui-tree-spread")), r = e.children("ul"), n = e.children("a"),
            l = function () {
                e.data("spread") ? (e.data("spread", null), r.removeClass("layui-show"), i.html(t.arrow[0]), n.find(".layui-icon:not(.btn_icon)").html(t.branch[0])) : (e.data("spread", !0), r.addClass("layui-show"), i.html(t.arrow[1]), n.find(".layui-icon:not(.btn_icon)").html(t.branch[0]))
            };
